...
export const СсылкаПротухла = {
  name: 'Приглашение — срок истёк',
  render: () => (
    <Demo
      url="https://link.qollab.kz/?command=invite&token=inv_expired"
      text="Вот ссылка на группу"
      note="Протухшая ссылка — не сбой приложения, а состояние объекта, поэтому ни красного, ни слова «ошибка», ни кода ответа. Формфактор другой: одна строка вместо полной карточки — нажимать нечего. Что за группа, по-прежнему не раскрываем."
    />
  ),
};

export const ПриглашениеОтозвали = {
  name: 'Приглашение — отозвали',
  render: () => (
    <Demo
      url="https://link.qollab.kz/?command=invite&token=inv_revoked"
      text="Скинул ссылку на канал"
      note="Отдельный текст под отзыв: бэкенд научится различать expired и revoked позже (раздел 7.2 плана), а копия под оба состояния уже готова."
    />
  ),
};

export const СсылкаНеРаботает = {
  name: 'Приглашение — состояние неизвестно',
  render: () => (
    <Demo
      url="https://link.qollab.kz/?command=invite&token=inv_broken"
      text="Залетай к нам"
      note="MVP-состояние: бэкенд отвечает просто invalid и не говорит, отозвали ссылку или вышел срок. Текст покрывает оба случая, не обещая того, чего не знает."
    />
  ),
};
