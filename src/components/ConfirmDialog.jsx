...
export default function ConfirmDialog({ title, text, confirmLabel, onConfirm, onCancel, danger, cancelLabel='Отмена' }) {
  return (
    <div className="cdlg-wrap">
      <button className="cdlg-scrim" onClick={onCancel} aria-label="Отмена" />
      <div className="cdlg" role="dialog" aria-modal="true" aria-label={title}>
        ...
        <div className="cdlg-actions">
          ...
          <button className="cdlg-btn" onClick={onCancel}>{cancelLabel}</button>
        </div>
      </div>
    </div>
  );
